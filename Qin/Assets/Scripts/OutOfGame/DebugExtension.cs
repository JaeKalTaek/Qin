...
public static class DebugExtension
{
	...
	public static void DebugMarker(Vector3 position, float size, Color color, float duration, bool depthTest = true)
	{
	...
	public static void DebugPlane(Vector3 position, Vector3 normal, float size, Color color, float duration, bool depthTest = true)
	{
		Vector3 v3;
	...
	public static void DebugVector(Vector3 position, Vector3 direction, float raySize, float markerSize, Color color, float duration = 0, bool depthTest = true)
	{
		Debug.DrawRay(position, direction * raySize, color, duration, false);
		DebugMarker(position + direction * raySize, markerSize, color, duration, false);
	}

	public static void DebugTriangle(Vector3 a, Vector3 b, Vector3 c, Color color)
	{
	...
	public static void DebugTriangle(Vector3 a, Vector3 b, Vector3 c, Color color, Transform t)
	{
	...
	public static void DebugwMesh(Mesh mesh, Color color, Transform t)
	{
		for (int i = 0; i < mesh.triangles.Length; i += 3)
		{
			DebugTriangle(mesh.vertices[mesh.triangles[i]], mesh.vertices[mesh.triangles[i + 1]], mesh.vertices[mesh.triangles[i + 2]], color, t);
		}
	}
	...
	/// </summary>
	/// <param name='position'>
	/// 	- The position of the sphere to draw.
	/// </param>
	///      <param name='radius'>
	/// 	- The radius of the drawn sphere.
	/// </param>
	///  <param name='color'>
	/// 	- The color of the drawn sphere.
	/// </param>
	...
	public static void DrawMesh(Mesh mesh, Vector3 position, float size, Color color)
	{
		Color oldColor = Gizmos.color;
		Gizmos.color = color;
		Gizmos.DrawMesh(mesh, position, Quaternion.identity, size * Vector3.one);
		Gizmos.color = oldColor;
	}

	public static void DrawWireMesh(Mesh mesh, Vector3 position, float size, Color color)
	{
		Color oldColor = Gizmos.color;
		Gizmos.color = color;
		Gizmos.DrawWireMesh(mesh, position, Quaternion.identity, size * Vector3.one);
		Gizmos.color = oldColor;
	}
	...
	public static void DrawBounds(Vector3 center, Vector3 extents, Color color)
	{
		float x = extents.x;
		float y = extents.y;
		float z = extents.z;
	...
	public static void HandleBounds(Bounds bounds, Color color)
	{
		Vector3 center = bounds.center;
		...
		Handles.DrawLine(ruf, luf);
		Handles.DrawLine(ruf, rub);
		Handles.DrawLine(luf, lub);
		Handles.DrawLine(rub, lub);

		Handles.DrawLine(ruf, rdf);
		Handles.DrawLine(rub, rdb);
		Handles.DrawLine(luf, lfd);
		Handles.DrawLine(lub, lbd);

		Handles.DrawLine(rdf, lfd);
		Handles.DrawLine(rdf, rdb);
		Handles.DrawLine(lfd, lbd);
		Handles.DrawLine(lbd, rdb);
